...
template <typename E> struct Err : public Clone<Err<E>> {
  Err()                     = delete;
...
public:
  ...
  auto _cloneImpl() const -> Result<T, E> {
    if (this->isOk()) {
      const auto& val = std::get<Ok<T>>(this->val);
...
template <typename E> struct Result<void, E> : public Clone<Result<void, E>> {
public:
  Result() = delete;
  Result(const Result&)
    requires(Copyable<E>)
